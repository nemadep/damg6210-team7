...
CREATE OR REPLACE PROCEDURE p_intial_reset AS
    ...
    c_shifts_transaction_table INT;
    c_guest_seq                INT;
    c_student_seq              INT;
    c_audit_seq                INT;
BEGIN
    ...
    SELECT
        func_is_table_created('shifts_audit_logs')
    INTO c_shifts_transaction_table
    FROM
        dual;
    ...
    IF c_audit_seq = 1 THEN
        dbms_output.put_line('**************************************');
        dbms_output.put_line('-------SEQUENCE - AUDIT_SEQ-------');
        dbms_output.put_line('---------Already Exists--------');
        dbms_output.put_line('**************************************');
    ELSE
        EXECUTE IMMEDIATE q'!CREATE SEQUENCE audit_seq!';
        dbms_output.put_line('Created : SEQUENCE - AUDIT_SEQ');
    END IF;
    ...
    IF c_shifts_transaction_table = 1 THEN
        dbms_output.put_line('**************************************');
        dbms_output.put_line('-------TABLE - SHIFTS_AUDIT_LOGS-------');
        dbms_output.put_line('---------Already Exists--------');
        dbms_output.put_line('**************************************');
    ELSE
        EXECUTE IMMEDIATE q'!CREATE TABLE shifts_audit_logs (
          id              NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 NOT NULL,
          ...
          CONSTRAINT shifts_audit_logs_pk PRIMARY KEY (id)
        )!';
        dbms_output.put_line('Created : TABLE - SHIFTS_AUDIT_LOGS');
    END IF;
