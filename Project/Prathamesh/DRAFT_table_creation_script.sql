...
BEGIN
    ...
    IF c_shifts_type_master = 1 THEN
        EXECUTE IMMEDIATE 'drop table shifts_type_master';
    END IF;
    IF c_student = 1 THEN
        EXECUTE IMMEDIATE 'drop table student';
    END IF;
    IF c_supervisor = 1 THEN
        EXECUTE IMMEDIATE 'drop table supervisor';
    ELSE
        EXECUTE IMMEDIATE q'!CREATE TABLE supervisor (
          supervisor_id NUMBER NOT NULL, 
          supervisor_name VARCHAR(50) NOT NULL, 
          supervisor_address VARCHAR(320) NOT NULL, 
          supervisor_contact VARCHAR(15) NOT NULL CHECK (
            supervisor_contact LIKE '(???)???-????'
          ), 
          supervisor_email VARCHAR(50) NOT NULL CHECK (
            REGEXP_LIKE (
              supervisor_email, '^(\S+)\@(\S+)\.(\S+)$'
            )
          ), 
    ...
    IF c_utility_type_master = 1 THEN
        EXECUTE IMMEDIATE 'drop table utility_type_master';
    END IF;
    IF c_utility = 1 THEN
        EXECUTE IMMEDIATE 'drop table utility';
    END IF;
    IF c_proctor = 1 THEN
        EXECUTE IMMEDIATE 'drop table proctor';
    END IF;
    IF c_guest = 1 THEN
        EXECUTE IMMEDIATE 'drop table guest';
    END IF;
    IF c_resident = 1 THEN
        EXECUTE IMMEDIATE 'drop table resident';
    END IF;
    IF c_dorm = 1 THEN
        EXECUTE IMMEDIATE 'drop table dorm';
    END IF;
    IF c_swipe_log = 1 THEN
        EXECUTE IMMEDIATE 'drop table swipe_log';
    END IF;
    IF c_shifts = 1 THEN
        EXECUTE IMMEDIATE 'drop table shifts';
    END IF;
    IF c_police = 1 THEN
        EXECUTE IMMEDIATE 'drop table police';
    END IF;
    IF c_incident = 1 THEN
        EXECUTE IMMEDIATE 'drop table incident';
    END IF;
    IF c_police_incident_mapping = 1 THEN
        EXECUTE IMMEDIATE 'drop table police_incident_mapping';
    END IF;
END;
/
